...
#define lastmodified "Last modified on Jun. 10, 2023"
#define webpage                                                                \
  "Find the last updates from "                                                \
  "[https://github.com/zhihuanli/gamma-gamma-coincidence-analysis/tree/"       \
  "master/Gate2]"
...
void setxrange(int xmin1 = xmin, int xmax1 = xmax) {
  st.xmin = xmin1;
  st.xmax = xmax1;
};
...
void gmxr(int xmin, int xmax, float ge1, float ge2 = -1, float ge3 = -1,
          float ge4 = -1);
void gmxr(int xmin, int xmax, TH1 *h1, TH1 *h2 = NULL, TH1 *h3 = NULL,
          TH1 *h4 = NULL);
...
void gandxr(int xmin, int xmax, float ge1, float ge2, float ge3 = -1,
            float ge4 = -1);
// add
void gadd(float ge1, float ge2, float ge3 = 0, float ge4 = 0, float ge5 = 0,
          float ge6 = 0);
void gaddxr(int xmin, int xmax, float ge1, float ge2, float ge3 = -1,
            float ge4 = -1, float ge5 = -1, float ge6 = -1);
// sub
void gsub(float ge1, float ge2, float ge3 = -1, float ge4 = -1);
void gsubxr(int xmin, int xmax, float ge1, float ge2, float ge3 = -1,
            float ge4 = -1);
...
void gate2() {
  ...
  if (gSystem->AccessPathName(st.rootfile.Data())) {
    cout << RED << "ROOT file: " << st.rootfile.Data() << " does not exist"
         << endl;
    gApplication->Terminate(0);
  }
  TFile *f = new TFile(st.rootfile.Data());
  if (!(hg2xy = (TH2 *)f->Get(st.histname.Data()))) {
    cout << RED << "ERROR!  No 2D-histogram in " << st.rootfile.Data()
         << " is detected !" << RESET << endl;
    gApplication->Terminate(0);
  }
...
void gand(float ge1, float ge2, float ge3, float ge4) {
  ...
  TH1F *hand = (TH1F *)hg2xy->ProjectionX(sname.Data(), hg2xy->GetXaxis()->FindBin(xmin), hg2xy->GetXaxis()->FindBin(xmax)); // copy hist structure
...
void gaddxr(int xmin, int xmax, float ge1, float ge2, float ge3, float ge4,
            float ge5, float ge6) {
  int x1 = st.xmin;
  int x2 = st.xmax;
  setxrange(xmin, xmax);
...
void gadd(float ge1, float ge2, float ge3, float ge4, float ge5, float ge6) {
  ...
  TH1F *hadd = (TH1F *)hg2xy->ProjectionX(sname.Data(), hg2xy->GetXaxis()->FindBin(xmin), hg2xy->GetXaxis()->FindBin(xmax)); // copy hist structure
  hadd->SetTitle(stitle);
  hadd->Reset("M"); //!!! reset maximum and minimum;

  for (int i = 0; i < npad; i++)
    hadd->Add(hadd, ha[i]);
  if (!tlhist->FindObject(hadd))
    tlhist->Add(hadd);
  ca[ic]->cd(npad + 1);
  icy = npad + 1;
  peaks(hadd);
...
void gsub(float ge1, float ge2, float ge3, float ge4) {
  ...
  TH1F *hsub = (TH1F *)hg2xy->ProjectionX(sname.Data(), hg2xy->GetXaxis()->FindBin(xmin), hg2xy->GetXaxis()->FindBin(xmax)); // copy hist structure

  hsub->SetTitle(stitle);
  hsub->Reset("M"); //!!! reset maximum and minimum;

  for (int i = 1; i < npad; i++)
    hsub->Add(ha[0], ha[i], 1, -1);
  if (!tlhist->FindObject(hsub))
    tlhist->Add(hsub);
  ca[ic]->cd(npad + 1);
  icy = npad + 1;
  peaks(hsub);
...
void saveHistData(TH1 *hist, TString sfile) {
  ...
  for (int i = 1; i <= hist->GetNbinsX(); i++) {
    fout << Form("%.1f   %.1f", hist->GetBinCenter(i), hist->GetBinContent(i))
         << endl;
  }
  fout.close();
  cout << "Write " << hist->GetName() << " to " << sfile.Data() << endl;
...
void saveCanvAll(TString sfile) {
  TFile *fout = new TFile(sfile.Data(), "UPDATE");
  TIter next(tlcanv);
  while (TCanvas *cc = (TCanvas *)next())
    cc->Write();
  // tlcanv->Write();//some error messages when browse through TCanvas in the
  // root file in MacOS.
  fout->Close();
  cout << "Write all existing histograms to " << sfile.Data() << endl;
}
...
void showSettings() {
  ...
  cout << GREEN << Form("Range of X/Y: %d - %d ", xmin, xmax) << RESET << endl
       << endl;
...
void help() {
  cout << YELLOW
       ...
       << endl;
  cout << RED << "Setup, " << RED
       << "changes will take effect for the next drawing." << endl;
  cout << CYAN
       << "   setxrange(int xmin1, int xmax1)        - Change the viewing "
          "range of x-axis for all histograms."
       << endl;
  cout << "   setnpeaks(int npeaks1=30)              - Change the number of "
          "peaks marked in a histogram."
       << endl;
  cout << "   setncanvas(int ncanvas=5)              - Change the maximum "
          "number of canvas avaliable."
       << endl;
  cout << "   setgatewidth(int dge=2)                - Change range of gate to "
          "peak-dge to peak+dge."
       << endl;
  cout << "   showSettings()                         - Show current parameter "
          "settings."
       << endl;
  ...
  cout << CYAN << "   gm(float ge1, [float ge2], [float ge3], [float ge4])"
       << endl;
  ...
  cout << CYAN << "   gand(float ge1,float ge2, [float ge3], [float ge4])"
       << endl;
  cout << "   gadd(float ge1,float ge2, [float ge3], [float ge4], [float ge5], "
          "[float ge6])"
       << endl;
  ...
  cout << CYAN << "   gmxr(int xmin,int xmax,double ge1, [double ge2], ...)"
       << endl;
  ...
  cout << CYAN
       << "   gandxr(int xmin,int xmax,float ge1,float ge2, [float ge3], "
          "[float ge4])"
       << endl;
  cout << "   gaddxr(int xmin,int xmax,float ge1,float ge2, [float ge3], "
          "[float ge4], [float ge5], [float ge6])"
       << endl;
  cout << "   gsubxr(int xmin,int xmax,float ge1,float ge2, [float ge3],[float "
          "ge4])"
       << endl;
...
void tpj(int icy) {
  ...
  ca[ic]->cd(icy);
  TString stitle =
      Form("Total Projection Sepctrum (%d - %d) keV ", st.xmin, st.xmax);
  TString sname = GetHistName("gtpj");
  TH1F *h = (TH1F *)hg2xy->ProjectionX(sname.Data(), hg2xy->GetXaxis()->FindBin(xmin), hg2xy->GetXaxis()->FindBin(xmax));
  h->SetTitle(stitle.Data());
  if (!tlhist->FindObject(h))
    tlhist->Add(h);
  peaks(h);
}
...
TString g(double ge, int icy1) {
  ...
  if (gea < hg2xy->GetXaxis()->FindBin(xmin))
    gea = hg2xy->GetXaxis()->FindBin(xmin);
  if (geb > hg2xy->GetXaxis()->FindBin(xmax))
    geb = hg2xy->GetXaxis()->FindBin(xmax);
  TString stitle =
      Form("Gated on %.1f keV (%.1f - %.1f)", ge, ge - st.dge, ge + st.dge);
...
void peaks(TH1 *h, Double_t thres) {
  ...
  if (ymin < 0 && ymin < -0.1 * ymax)
    ymin = -0.1 * ymax; // ymin=0.01;
  cout << CYAN << ic << " " << icy << " " << h->GetName() << "  ["
       << h->GetTitle() << "]" << RESET << endl;
  ...
  while (nloop < 1000) {
    ...
    if (nfound > st.npeaks)
      ...
      thres -= 0.0001;
    if (thres < 0 || abs(nfound - st.npeaks) < 3)
      break;
    nloop++;
  }
  TPolyMarker *pm =
